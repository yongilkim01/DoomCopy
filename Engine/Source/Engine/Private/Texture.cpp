...
void UTexture::CreateAsset(const D3D11_TEXTURE2D_DESC& InitTextureDesc)
{
	...
	UGameEngine::GetDevice()->CreateTexture2D(&TextureDesc, nullptr, &Texture2D);
...
{
	if (S_OK != UGameEngine::GetDevice()->CreateRenderTargetView(Texture2D.Get(), nullptr, &RenderTargetView))
	{
...
{
	if (S_OK != UGameEngine::GetDevice()->CreateShaderResourceView(Texture2D.Get(), nullptr, &ShaderResourceView))
	{
...
{
	if (S_OK != UGameEngine::GetDevice()->CreateDepthStencilView(Texture2D.Get(), nullptr, &DepthStencilView))
	{
